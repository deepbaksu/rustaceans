...
pub fn convert(parse_result: ParseResult) -> Result<String, String> {
    match (parse_result.from, parse_result.to) {
        (Unit::DateTimeWithTimeZone(dt_wrapper), Unit::TimeZoneOnly(tz)) => {
            let from_time: DateTime<_> = dt_wrapper
                ...
                .single().ok_or(format!("the left datetime cannot be converted to DateTime. It's caused when the datetime cannot be converted into timezone. Input: {:?}", dt_wrapper))?;
